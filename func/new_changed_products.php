    ...
    while ( $row = mysql_fetch_array($rs) )
      {
        ...
        $display .= '<h3 id="sub'.$subcategory_id.'">'.$subcategory_name.'</h3>';
        $sqlp = '
          SELECT
            '.TABLE_PRODUCT_PREP.'.*,
                    ...
                    while ( $row = mysql_fetch_array($result2) )
                      {
                        ...
                        if (SHOW_ACTUAL_PRICE)
                        {
                          ...
                          $unit_price_old /= (1 - (UNIVERSAL_MARGIN + $product_margin_old));
                          $unit_price_old = round($unit_price_old, 2);
                        }
                        ...
                        if( $extra_charge_old )
                          {
                            $extra_old = '<br>Extra charge: '.CURSYM.number_format($extra_charge_old, 2).'/'.$ordering_unit_old;
                          }
                        else
                          {
                            $extra_old = '';
                        ...
                        $ordering_unit_old = stripslashes($ordering_unit_old);
                        $display .= Inflect::singularize($ordering_unit_old);
                        $display .= '</td>';

                        $display .= '
                              <td>'.CURSYM.number_format($unit_price_old, 2).'/'.$pricing_unit_old.'</td>
                               <td valign="center" id="'.$product_id.'">OLD LISTING '.$display_donot.'</td>';
                        $display .= '
                            </tr>';
                        $display .= '
                            <tr>
