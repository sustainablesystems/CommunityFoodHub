...
while ( $row = mysql_fetch_array($rs) )
  {
    ...
    $display .= "<tr bgcolor=#DDDDDD><td colspan=\"8\" id=\"$product_id\">
    <font size=5>$product_name (Product ID# $product_id)</font><br>
    <b>".CURSYM.number_format($item_price, 2)."/$pricing_unit</b>
    </td></tr>";
    ...
    while ( $row = mysql_fetch_array($resultpr) )
      {
        ...
        $display_label .= (convert_route_code($basket_id, $member_id, $last_name, $first_name, $business_name, $a_business_name, $delcode_id, $delcode_id, $deltype, $truck_code, $storage_code, $show_mem, $show_mem2, $product_name, $quantity, $ordering_unit, $product_id, $item_price, $delcode)).'<br>';
        $display_label .= stripslashes($show_mem2).'<br>';
        // Don't show producer info for suppliers
        if ( !$is_supplier )
          {
