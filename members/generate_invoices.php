...
function cust_generate_start() {
  ...
  $.post("<?php echo PATH; ?>ajax/compile_customer_invoices.php", { query_data: "delete_html:"+"<?php echo $delivery_id; ?>" }, function(data) {
...
function compile_customer_invoices() {
  ...
  if (i < c_arrElements.length) {
    ...
    if (c_arrElements[i].attributes["class"].value == 'c_incomplete') {
      ...
      $.post("<?php echo PATH; ?>ajax/compile_customer_invoices.php", { query_data: ""+element_id+":<?php echo $delivery_id; ?>" }, function(data) {
        if(data == "GENERATED_INVOICE") {
          ...
          if (i == c_arrElements.length) {
            ...
            <?php if (USE_HTMLDOC) { echo 'cust_generate_pdf();'; } else { ?>
...
function cust_generate_pdf() {
  ...
  $.post("<?php echo PATH; ?>ajax/compile_customer_invoices.php", { query_data: "delete_pdf:"+"<?php echo $delivery_id; ?>" }, function(data) {
    if (data != "DELETED_PDF") {
      alert ("ERROR C3: "+data+" \r\nPlease try again");
      }
    })
  $.post("<?php echo PATH; ?>ajax/compile_customer_invoices.php", { query_data: "html2pdf:"+"<?php echo $delivery_id; ?>" }, function(data) {
    if(data != "HTML2PDF") {
      alert ("ERROR C4: "+data+" \r\nPlease try again");
      }
...
function prod_generate_start() {
  // Delete the old html file before continuing
  $.post("<?php echo PATH; ?>ajax/compile_producer_invoices.php", { query_data: "delete_html:"+"<?php echo $delivery_id; ?>" }, function(data) {
    if (data != "DELETED_HTML") {
      alert ("ERROR P1: "+data+" \r\nPlease try again");
      }
    })
...
//      alert ("DATA: "+data);
      ...
      $.post("<?php echo PATH; ?>ajax/compile_producer_invoices.php", { query_data: ""+element_id+":<?php echo $delivery_id; ?>" }, function(data) {
        if(data == "GENERATED_INVOICE") {
          ...
          if (i == p_arrElements.length) {
            ...
            <?php if (USE_HTMLDOC) { echo 'prod_generate_pdf();'; } else { ?>
...
function prod_generate_pdf() {
  ...
  // Delete the old pdf file before continuing
  $.post("<?php echo PATH; ?>ajax/compile_producer_invoices.php", { query_data: "delete_pdf:"+"<?php echo $delivery_id; ?>" }, function(data) {
    if (data != "DELETED_PDF") {
      alert ("ERROR P3: "+data+" \r\nPlease try again");
      }
    })
  $.post("<?php echo PATH; ?>ajax/compile_producer_invoices.php", { query_data: "html2pdf:"+"<?php echo $delivery_id; ?>" }, function(data) {
    if(data != "HTML2PDF") {
      alert ("ERROR P4: "+data+" \r\nPlease try again");
      }
...
<div id="left-column">
  ...
  <div id="customerBox">
    <div class="customerList" id="customerList">
      <ul>
        <li><div class="c_list_header c_list_cid">ID</div><div class="c_list_header c_list_name">Collection Location [Name]</div></a></li>';
...
while($row = mysql_fetch_array($result))
  {
    ...
    echo stripslashes ('          <li id="basket_id'.$basket_id.'" class="c_complete"><a href="orders_invoice.php?delivery_id='.$delivery_id.'&basket_id='.$basket_id.'&member_id='.$member_id.' " target="_blank"><div class="c_list_cid">'.$ready_begin.$row['member_id'].$ready_end.'</div><div class="c_list_name">'.$delcode_id.': '.$delcode_id.' ['.$last_name.', '.$first_name.']</div></a></li>');
  }
