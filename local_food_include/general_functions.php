...
function get_price($ordering_unit, $pricing_unit_price)
{
  // HACK! When displaying the price to the customer,
  // use the ordering unit and adjust the price accordingly if the
  // ordering unit contains the words "half" or "quarter".
  // E.g. if the pricing unit is "kilo" and the ordering unit is "half kilo"
  // then halve the price.
  $half_pos = strpos(strtolower($ordering_unit),"half");  
  if ($half_pos === false)
  {
    $quarter_pos = strpos(strtolower($ordering_unit),"quarter");
    if ($quarter_pos === false)
    {
      $ordering_price = $pricing_unit_price;
    }
    else
    {
      $ordering_price = $pricing_unit_price / 4.0;
    }
  }
  else
  {
    $ordering_price = $pricing_unit_price / 2.0;
  }
...
function get_auth_types($auth_type_csv)
  {
    // First reset all auth values
    foreach (explode (',', VALID_AUTH_TYPES) as $auth_type)
      {
        $authorization[$auth_type] = false;
      }
    // Then set valid auth values
    foreach (explode (',', $auth_type_csv) as $auth_type)
      {
        $authorization[$auth_type] = true;
      }
