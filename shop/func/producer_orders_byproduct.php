...
while ( $row = mysql_fetch_array($rs) )
  {
    ...
    while ( $row = mysql_fetch_array($resultpr) )
      {
        ...
        if ( $extra_charge )
          {
            ...
            $product_charge = CURSYM.number_format($extra_charge_calc, 2);
          }
        else
          {
            $product_charge = "";
        ...
        if ( $item_total_price )
          {
            $product_total_price = CURSYM.$item_total_price;
          }
    ...
    $producer_orders_byproduct .= '
          <tr>
            <td colspan="8">Product subtotal: '.CURSYM.number_format($subtotal_pr, 2).'</td>
          </tr>';
  }
$producer_orders_byproduct .= '
      </table>';
