...
if ( ! $_POST['updatevalues'] )
  {
    $_POST['sort1'] = 'Collection Location';
    $_POST['sort2'] = 'Subcategory';
    $_POST['sort3'] = 'Product Name';
    $_POST['sort4'] = 'Member Name';
  }
...
while ( $row = mysql_fetch_array($resultpr) )
  {
    ...
    if ( $show_form )
      {
        // Don't show stock information at the moment
        /*
...
if ( $extra_charge != 0 )
  {
    ...
    // Don't show the member contact info or route info (more detail than we want here).
    $header_member = '
      Member: <b>'.$show_mem.'</b> (#'.$member_id.')';
    $header_member_name = '
      Member: <b>'.$show_mem.'</b> (#'.$member_id.')';
    $header_delivery_code = '
      <b>Collection Location:</b> '.$delcode_id.'
      <br>'.$delcode;
    ...
    $header_product_name = '
      Product: <b>'.$product_name.'</b> ('.$product_id.')
      <br><font size="-1">'.$display_price.'</font>';
    ...
    if ( $sort_array[$sort1]['value'] != $$sort_array[$sort1]['var_name'] )
      {
        ...
        if ( $sort_level == 1 )
          {
            // Remove column headings - just want quantity and price (item total)
            /*$producer_orders_multi .= '
