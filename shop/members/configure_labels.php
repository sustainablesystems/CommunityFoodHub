...
if ($show_list == true)
  {
    ...
    foreach (array_keys (array_flip ($defined_labels)) as $defined_label)
      {
        echo '
          <tr style="background-color:#ddc;">
            ...
            <td style="text-align:center;padding:0.5em 3em;"><a href="'.$_SERVER['PHP_SELF'].'?action=configure&label_name='.base64_decode (strtr ($defined_label, '-', '=')).'">Configure<br>label</a></td>
            <td style="text-align:center;padding:0.5em 3em;"><a href="'.$_SERVER['PHP_SELF'].'?action=select&label_name='.base64_decode (strtr ($defined_label, '-', '=')).'">Select<br>label</a></td>
            <td style="text-align:center;padding:0.5em 3em;"><a href="'.$_SERVER['PHP_SELF'].'?action=delete_query&label_name='.base64_decode (strtr ($defined_label, '-', '=')).'">Delete<br>label</a></td>
          </tr>';
      }
    echo '
      </table><br>';
    if ($_GET['action'] != 'configure')
      {
        echo '
          Add a <a href="'.$_SERVER['PHP_SELF'].'?action=configure">new label</a> type.
          <br><br>';
      }
    echo '
    ...
    echo '
      ...
      <table style="background-color:#ddc;border:1px solid #888;">
        ...
        <tr>
          ...
          <td rowspan="10"><img src="grfx/label_calibration.gif"</td>
        </tr>
        <tr>
