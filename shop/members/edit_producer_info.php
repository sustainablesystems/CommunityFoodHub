...
if($authorization['administrator'] === true && $_GET['producer_id'])
  {
...
while ( $row = mysql_fetch_array($rsr) )
  {
    ...
    $show_name_logged_in_member = $show_name;
    ...
    $show_name = $show_name_logged_in_member;

    if ($address_line1) { $display .= "$address_line1<br>"; }
    if ($address_line2) { $display .= "$address_line2<br>"; }
    if ($address_line1) { $display .= "$city, $state $zip<br>"; }

    if ($email_address)
      {
        $display .= "<a href=\"mailto:$email_address\">$email_address</a><br>";
      }
    if ($home_phone) { $display .= "$home_phone (home)<br>"; }
    if ($work_phone) { $display .= "$work_phone (work)<br>"; }
    if ($mobile_phone) { $display .= "$mobile_phone<br>"; }
    if ($fax) { $display .= "$fax (fax)<br>"; }
    if ($toll_free) { $display .= "$toll_free (toll free)<br>"; }
    if ($home_page) { $display .= "<a href=\"http://$home_page\" target=\"_blank\">$home_page</a><br>"; }
...
<h1 align="center">Edit Producer Information</h1>
...
<div align="right">
  <b><a href="<?php echo BASE_URL.PATH.'producers/'.$producer_id_lower;?>.php?members=true" target="_blank">Click here to see the live webpage</a></b>
  <?php echo $fulllist_link;?>
</div>

<blockquote>
  <form method="post" action="<?php echo $_SERVER['PHP_SELF'];?>?producer_id=<?php echo $producer_id; ?>">

    <font size="3"><b>Product Types:</b></font> (max. 150 characters, use keywords like lettuce, berries, soap, etc.)<br>
    ...
    <font size="3"><b>Practices:</b></font> (Use this space to describe your standards and practices. For example, if you use all natural products, whether or not the processing of your products is done in a health-inspected kitchen/facility, etc.)<br>
    <textarea name="practices" cols="75" rows="17"><?php echo $general_practices;?></textarea>
    <br><br>

    <font size="3"><b>Additional Information:</b></font> (Use this space for anything that isn&rsquo;t covered in these other sections.)<br>
