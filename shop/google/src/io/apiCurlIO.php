...
class apiCurlIO implements apiIO {
  private static $DEFAULT_CURL_PARAMS = array (
      ...
      // IRH: No CA cert bundle on localhost with cURL - see my comments below.
  ...
  public function makeRequest(apiHttpRequest $request) {
    ...
    // IRH: cURL must be configured with the CA certificate for the Google APIs server
    // By default, cURL doesn't come with any CA certicates, see doc here:
    // http://curl.haxx.se/docs/sslcerts.html
    //
    // On the live site, cURL will already be configured for us by the ISP
    //
    // For localhost, I got a CA certificate bundle from here (as suggested on the page above):
    // http://curl.haxx.se/docs/caextract.html
    //
    // This is the exception we get otherwise:
    //apiIOException: HTTP Error: (0) SSL certificate problem, verify that the CA cert is OK. Details: error:14090086:SSL routines:SSL3_GET_SERVER_CERTIFICATE:certificate verify failed
    //curl_setopt($ch, CURLOPT_CAINFO, '/xampp/htdocs/shop/google/src/io/certs/cacert.pem');
    //
    // N.B. Another option is to set CURLOPT_SSL_VERIFYPEER to FALSE in the $DEFAULT_CURL_PARAMS (top of file)
