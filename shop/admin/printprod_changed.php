...
$sql = '
  ...
  WHERE
    ...
    AND '.TABLE_PRODUCT_PREP.'.donotlist != "2"
    AND '.TABLE_PRODUCT_PREP.'.changed = "1"
  GROUP BY
    '.TABLE_PRODUCT_PREP.'.subcategory_id
  ORDER BY
    sort_order ASC,
    ...
    while ( $row = mysql_fetch_array($resultp) )
      {
        ...
        // If the producer's a supplier, don't show their name
        if ( !$is_supplier )
        {
          $display .= '
            <tr>
        ...
        while ( $row = mysql_fetch_array($result) )
          {
            ...
            while ( $current_product_id != $product_id )
              ...
              <tr>
                ...
                <td valign="top">'.$font2.' <b>'.stripslashes($product_name).'</b> - Order number of '.Inflect::pluralize($ordering_unit).'. '.$show_details.' '.$extra.'</font></td>
                <td valign="top" align=center>'.$font2.' '.CURSYM.number_format($unit_price, 2).'/'.$pricing_unit.'</font></td>
...
<html>
<head>
<title><?php echo ucfirst (SITE_NAME); ?> Product List</title>
</head>
<body bgcolor="#FFFFFF">
<!-- CONTENT BEGINS HERE -->
<hr color="#000000" noshade size="2" width="100%">
<font size="+3"><?php echo ucfirst (SITE_NAME); ?> <?php echo $date_today;?> List of Changed Products Since Last Month</font>
<hr color="#000000" noshade size="2" width="100%">
<?php echo $display;?>
<!-- CONTENT ENDS HERE -->
</body>
