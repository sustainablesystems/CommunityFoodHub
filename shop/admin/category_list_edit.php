...
function listCategoriesEdit ($parent_id, $level, $parent_category_name)
  {
    ...
    if (mysql_affected_rows($connection) > 0)
      { // There are more categories (or subcategories) to look at
        ...
        while ($row = mysql_fetch_array($sql))
          {
            ...
            if ($prior_category_id)
              {
                $move_up_markup = '<a href="'.PATH.'admin/category_list_edit.php?action=Exchange&category_id1='.$category_id.'&category_id2='.$prior_category_id.'">&uarr;</a>'; }
            else
              {
                $move_up_markup = '&nbsp;&nbsp;';
              };
            ...
            $move_down_markup = '<a href="'.PATH.'admin/category_list_edit.php?action=Exchange&category_id1='.$category_id.'&category_id2=CrYpTiCsTrInG">&darr;</a>';

            if ($taxable_category == 1) $taxable_category_flag = '<span class="taxable">*</span>';
            if ($taxable_category == 0) $taxable_category_flag = '';

            $list_markup .= '<li class="cat'.$level.'">'.$move_up_markup.'&nbsp;'.$move_down_markup.'&nbsp;&nbsp;&nbsp;<a href="'.PATH.'admin/category_list_edit.php?action=Edit&category_id='.$category_id.'">'.$category_name.' '.$taxable_category_flag.'</a>'."\n";
        ...
        $list_markup .= '<li class="cat'.$level.'">[<a href="'.PATH.'admin/category_list_edit.php?action=AddCategory&parent_id='.$parent_id.'">Add new category under <u>'.$parent_category_name.'</u></a>]</li>'."\n";
        $list_markup .= "</ul>\n";
        ...
        while ($row2 = mysql_fetch_array($sql2))
          {
            ...
            $list_markup .= '<li class="cat'.$level.'">&raquo; <a href="'.PATH.'admin/category_list_edit.php?action=Edit&category_id='.$parent_id.'&subcategory_id='.$subcategory_id.'">'.$subcategory_name.' '.$taxable_subcategory_flag.'</a>'.$categories_exist.'</li>'."\n";
          }
        $list_markup .= '<li class="cat'.$level.'">&raquo; [<a href="'.PATH.'admin/category_list_edit.php?action=AddSubcategory&parent_id='.$parent_id.'">Add new subcategory under <u>'.$parent_category_name.'</u></a>]</li>'."\n";
        $list_markup .= "</ul>\n";
      }
