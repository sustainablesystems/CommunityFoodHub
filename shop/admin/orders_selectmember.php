...
if ( $qy == 'jk' )
  {
    ...
    if ( !$delcode_id )
      {
        $message2 = '<h3>Please enter a Collection Location.</h3>';
      }
    elseif ( !$deltype )
      {
        $message2 = '<h3>Please choose Home, Work, or Collection.</h3>';
      }
    ...
    if (( $selectmember == 'pass' ) && $delcode_id && $deltype && $payment_method)
      {
        ...
        if ( $num4 == 1 )
        ...
        else // Open a basket
          {
            ...
            $basket_id = open_basket($member_id, $target_delivery_id, $delcode_id, $deltype, $payment_method);
...
$display_deltype = '
  <input type="radio" name="deltype" value="H" disabled><font color="grey">Home Delivery</font>
  <input type="radio" name="deltype" value="W" disabled><font color="grey">Work Delivery</font>
  <input type="radio" name="deltype" value="P" checked>Collection';
$sqldc = '
  SELECT *
  FROM
    '.TABLE_DELCODE.'
...
while ( $row = mysql_fetch_array($rs) )
  {
    ...
    $delcode_first = '
                    <option value="">Choose a collection option</option>';
    $display_delcode .= '
