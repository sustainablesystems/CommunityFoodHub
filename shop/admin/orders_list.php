...
$sql = '
  ...
  GROUP BY
    '.TABLE_BASKET_ALL.'.member_id
  ORDER BY
    '.TABLE_BASKET_ALL.'.submitted ASC,
    last_name ASC';
...
while ( $row = mysql_fetch_array($rs) )
  {
    ...
    $subtotal = '';
    $display .= ($submitted ? '<tr bgcolor="#FFFFFF">' : '<tr bgcolor="#FFBBBB">');
    ...
    if ( $basket_delivery_id != $delivery_id )
    ...
    else
      {
        if ( $finalized != 1 )
          {
            $display .= '<td valign="top"><font size="2"><a href="customer_invoice.php?delivery_id='.$delivery_id.'&basket_id='.$basket_id.'&member_id='.$member_id.'" target="_new">View Temp. Inv.</a></font></td>';
          }
        else
          {
            $display .= '<td></td>';
    ...
    else
      {
        if ( $finalized != 1 )
        ...
        else
          {
            ...
            $display .= '<td valign="top"><font size=-2><a href="'.$PHP_SELF.'?fin=unfinalize&delivery_id='.$delivery_id.'&basket_id='.$basket_id.'&member_id='.$member_id.'#'.$basket_id.'">Unfinalise to Edit</a></font></td>';
          }
      }
    // Indicate if the basket hasn't been submitted and provide a link to the basket to check it
    if ($submitted)
      {
        $display .='<td valign="top" align="center">Yes'
          .($finalized == 1 ? '</td>'
            : ' - <a href="orders.php?delivery_id='.$delivery_id.'&basket_id='.$basket_id
              .'&member_id='.$member_id.'">View/Edit basket</a></td>');
      }
    else
      {
        $display .='<td valign="top" align="center"><b>No</b> - <a href="orders.php?delivery_id='
          .$delivery_id.'&basket_id='.$basket_id.'&member_id='.$member_id.'">View/Submit basket</a></td>';
      }
...
?>
      ...
      Click to <b>finalise invoices</b> for members id #:
      ...
      <a href="finalize.php?delivery_id=<?php echo $delivery_id;?>&ms=1001&mf=1100">1001-1100</a> |
...
<table bgcolor="#DDDDDD" cellpadding="2" cellspacing="2" border="0">
  <tr bgcolor="#AEDE86">
    ...
    <th>Finalised After Delivery</th>
    <th>UnFinalise</th>
