            ...
            if ( $row = mysql_fetch_object ($sql) )
              {
                ...
                $msg .= "Many of your questions can be answered at our web site: ".HOMEPAGE_URL."\n";
    ...
    '.TABLE_MEMBERSHIP_TYPES.'.membership_class,
      (
        ...
        WHERE
          ...
          AND '.TABLE_TRANSACTIONS.'.transaction_type != 64 /* Do not include Per-order credit/fee values */
      ) AS membership_dues
  FROM
    '.TABLE_MEMBER.'
  LEFT JOIN
...
while ( $row = mysql_fetch_array($sql) )
  {
    ...
    if ($membership_dues != 0 || $pending == 1)
      {
        ...
        //not yet approved
            $checked['pending'] = '';
            $checked['approved'] = '';
            if ( $pending == 0 || $pending == 2 ) //2 is approved but not paid
              {
        ...
        //not yet paid
        $display_dues = '';
        if ( $membership_dues < 0) $display_dues = '<span class="overpaid">'.CURSYM.number_format (-$membership_dues, 2).' (credit)</span>';
        if ($membership_dues > 0) $display_dues = '<span class="due">'.CURSYM.number_format ($membership_dues, 2).' due</span>';
        $payment_display = '';
        ...
        $display .= '
          ...
          Paid: '.CURSYM.' <input type="text" name ="payment_amount'.$counter.'" size="6" maxlength="8"><br /><br />
